...
def group_by_MVN(csv_path='/home/ise/eran/eran_D4j/results/smart/all_df_merg.csv'):
    ...
    print list(df)
    df.drop(['buggy_class_err', 'buggy_class_fail', 'fix_class_err', 'time_budget', 'fix_class_fail', 'name', 'fix_err',
             'buggy_err'], axis=1, inplace=True)
...
def group_by_unkillabe(p_name='Math'):
    ...
    df_bug_ID_scop_unkillabe['sum_GROUP'] = df_bug_ID_scop_unkillabe.groupby(['bug_id', 'project_id'])[
        'rep_avg'].transform('sum')
    df_bug_ID_scop_unkillabe['MAX_search_time'] = df_bug_ID_scop_unkillabe.groupby(['bug_id', 'project_id'])[
        'time_budget'].transform('max')
    ...
    for dir_i in bug_dirs_Test:
        ...
        if len(d_diff_results) > 0:
            ...
            for klass in buggy_test_case:
                list_d.append(
                    {'project': project_id, 'father_dir': father_dir, 'bug_ID': bug_id, 'time_budget': time_budget,
                     'iteration_id': it_id, 'dir': 'buggy', 'class': klass[:-7]})
            for klass in fixed_test_case:
                list_d.append(
                    {'project': project_id, 'father_dir': father_dir, 'bug_ID': bug_id, 'time_budget': time_budget,
                     'iteration_id': it_id, 'dir': 'fixed', 'class': klass[:-7]})
        else:
...
def uniform_vs_prefect_oracle(csv_path, allocation_mode_name='allocation_mode', name_modfiy='Math_faulty_comp.csv'):
    ...
    df_merge['orcale_acutalle_time_budget'] = df_merge.apply(my_test1, time_budget_arr=arr_time_budget, out=out,
                                                             df=df_q_deep, axis=1)
...
def second_pahse_merge_oracle_out(df_oracle_out, p_name, dir_out):
    ...
    df_oracle_out_target['package_info'] = df_oracle_out_target.apply(
        lambda row: make_comparison(row, df_oracle_out_not_target), axis=1)
    df_oracle_out_target['package_total_time_gen'] = df_oracle_out_target['package_info'].apply(
        lambda x: str(x).split('|')[-1])
    df_oracle_out_target['package_time_budget'] = df_oracle_out_target['package_info'].apply(
        lambda x: str(x).split('|')[-2])
    df_oracle_out_target['package_kill'] = df_oracle_out_target['package_info'].apply(
        lambda x: '|'.join(str(x).split('|')[:-2]))
    for col in list_rep:
        df_oracle_out_target["package_{}".format(col)] = df_oracle_out_target['package_kill'].apply(
            lambda val: split_info(val=val, col_name=col))
        ...
        for file_i in list_files:
            ...
            if True is False:
                ...
                if name.__contains__('bug'):
                    d_list.append({'bug_id': bug_id, 'project_id': project_id, 'size_scope': len(classes_list),
                                   'classes': '-'.join(classes_list), 'scope': 'target'})
                else:
                    d_list.append({'bug_id': bug_id, 'project_id': project_id, 'size_scope': len(classes_list),
                                   'classes': '-'.join(classes_list), 'scope': scope_i})
...
def get_df_size_actual_out(p_dir_path='/home/ise/D4j/oracle/log_generated_tests', out_dir_path='/home/ise/D4j'):
    ...
    if os.path.isdir(p_dir_path):
        ...
        for file_i in res_files:
            ...
            list_dico.append(
                {'project_id': project_id, 'bug_id': bug_id, 'test_id': iter_id, 'actual_size': size_lines})
    if len(list_dico) > 0:
...
def get_package_csv(root_dir):
    ...
    for dir in dirs_bug_id:
        ...
        if os.path.isfile("{}/log/bug_classes.txt".format(dir)):
            ...
            size_of_faulty = -1
        list_of_d.append(
            {'bug_id': bug_id, 'project': proj_name, 'size_package': size_of_package, 'size_faulty': size_of_faulty})
...
def itrate_rows(row, dico_l):
    ...
    for item in str(list_fail_buggy)[1:-1].split(','):
        ...
        dico_l.append(
            {'bug_ID': bug_id, 'dir': 'buggy', 'project': project_id, 'time_budget': time_b, 'TEST_NAME': test_class,
             'class': item})
...
def get_size_classes_csv(id, p_name, out_csv_path, tmp_dir='/tmp',
                         root_d4j='/home/ise/programs/defects4j/framework/bin/defects4j'):
    '''
...
def get_LOC(class_path):
    ...
    try:
        ...
        print '[Error] Exception as been ocuur while trying to conver {} to an int, in the function get_LOC'.format(
            size)
        size = None
    return size
    ...
    for bug_i in id_list_bug:
        ...
        for rep_i in range(1, max_all_rep + 1):
            ...
            d_list_res.append({'bug_ID': bug_i, 'kill_val': val_random, 'method': 'random', 'rep_sampled': rep_i,
                               'size_suite': size_tset_suite, 'size_faulty_classes': size_faulty_suite})
            d_list_res.append({'bug_ID': bug_i, 'kill_val': val_target, 'method': 'target', 'rep_sampled': rep_i,
                               'size_suite': size_tset_suite, 'size_faulty_classes': size_faulty_suite})
    df_res = pd.DataFrame(d_list_res)
...
def get_bug_d4j_major(p_name='Mockito', fp_dir='/home/ise/tmp_d4j/out_pred/out',
                      dir_rep_exp='/home/ise/eran/out_csvs_D4j/rep_exp', config_dir='/home/ise/tmp_d4j/config',
                      out='/home/ise/tmp_d4j/out', major=False):
    '''
    ...
    for i in range(4):
        df['MAJOR_{}'.format(i)] = df['TAG_NAME'].apply(
            lambda x: " " if len(str(x).split('_')) <= i else str(x).split('_')[i])
    print list(df)
    if major is False:
        ky = d_csv_fp.keys()[0]
        df['MAJOR_1'] = ky
...
def pivoting(p_name='commons-math', var=2):
    ...
    for method in methods:
        ...
        for item in items_values:
            ...
            if var == 1:
                d[method][item] = "{}".format(int(unique_kill))
            elif var == 2:
    ...
    if var == 1:
        df_fin.to_csv("{}/out/pivot_v1.csv".format(father))
    if var == 3:
        df_fin.to_csv("{}/out/pivot_v3.csv".format(father))
    else:
        df_fin.to_csv("{}/out/pivot_v2.csv".format(father))
    return df_fin


def normalize_FP_rows(df):
    # df = pd.read_csv(data_csv,index_col=0)
    print list(df)
    # group_weights = df.groupby('bug_ID')['FP'].aggregate(sum)
    # df['normalized_FP'] = df.apply(lambda row: row['FP'] / group_weights.loc[row['bug_ID']][0], axis=1)
    # df['norm_FP'] = df['FP'] / df.groupby('bug_ID')['FP'].transform('sum')
    # df.drop(['FP'], axis=1,inplace=True)
    # df = df.rename(columns={'norm_FP': 'FP'})

    df['norm_LOC'] = df['LOC_P'] / df.groupby('bug_ID')['LOC_P'].transform('sum')
    df.drop(['LOC_P'], axis=1, inplace=True)
    df = df.rename(columns={'norm_LOC': 'LOC_P'})
