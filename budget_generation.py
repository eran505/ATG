...
def get_instance_bug_and_fix(bug_lis, fix_lis, delimiter):
    ...
    if len(fix_lis) != len(bug_lis):
        print ("-----Errorbox: fix list and bug list not in the same size fix_size={0} bug_size={1}-----".format(
            len(fix_lis), len(bug_lis)))
    for item in bug_lis:
        for it in item:
            name_dot = it[0] + '/' + it[1]
...
def get_all_class_v2(root):
    ...
    new_list = [i for i in list_java if
                str(i).__contains__("$") == False and str(i).__contains__("package-info") == False]
    new_list_sp = []
    for p in new_list:
...
def regression_test(evo_name, evo_path, list_fixed, list_buggy, budget_dic, dis_path, lower_b, upper_b, seed, total_b,
                    m_class):  # fixed = original |||| bug =  regression
    ...
    for test_case in result_dict.keys():
        ...
        command = "{0} -projectCP {1} -Dregressioncp={2} -class {5} {3} {4}".format(evo_string, fix_path, bug_path,
                                                                                    criterion, all_p, test_case)
...
def regression_testing_handler(
        bug_obj):  # params [ -buggy_path  -fixed_path -[U/F/O/A] -path_csv_FP -mode -output_path -evo_version -evo_path -time_budget -Lower_b -Up_b  ]
    print "[regression]"
    ...
    if run_var == 'A':
        ...
        comp = [run_var]
    if "FP" in comp:
        fp_budget, d = get_time_fault_prediction(csv_path_fp, 'FileName', 'prediction', out_path, upper_b, lower_b,
                                                 total_budget_class)
    ...
    for i in range(1):
        ...
        for parm in comp:
            localtime = time.asctime(time.localtime(time.time()))
            dir_name = "{0}_D4j_{1}_t={2}_it={3}_b={4}_p={5}".format(str(parm), str(localtime), str(total_budget_class),
                                                                     str(i), str(bug_obj.id), str(bug_obj.p_name)[:2])
            full_dis = mkdir_Os(out_path, dir_name)
            if full_dis == 'null':
                print('cant make dir')
                exit(1)
            if str(parm) == 'FP':
                regression_test(evo_version, evo_path, target_list_fixed_list_package, target_list_buggy_list_package,
                                fp_budget, full_dis, lower_b, upper_b, seed, total_budget_class, m_class)
            elif str(parm) == 'U':
                mode_budget = {'mode': 'U'}
                regression_test(evo_version, evo_path, target_list_fixed_list_package, target_list_buggy_list_package,
                                mode_budget, full_dis, lower_b, upper_b, seed, total_budget_class, m_class)
            elif str(parm) == 'OR':
                mode_budget = {'mode': 'OR'}
                regression_test(evo_version, evo_path, [target_list_fixed_list_classes],
                                [target_list_buggy_list_classes], mode_budget, full_dis, lower_b, upper_b, seed,
                                total_budget_class, m_class)
